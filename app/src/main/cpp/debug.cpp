...
void WriteTextLogBottom(int level, const char *log) {
    if (level <= g_log_file_level) {
        ...
        if (stat(g_logFilePath.c_str(), &info) != 0) {
            ...
            if (fp == nullptr) {
                ALOGE("%s, fopen(w) %s fail, err:%d", __func__, g_logFilePath.c_str(), errno);
                return;
            }
        ...
        if (info.st_size >= LOG_FILE_MAX_SIZE)// loop write
        {
            ...
            if (nullptr == fp) {
                ALOGE("%s, fopen(r+) %s fail, size:%ld, err:%d", __func__, g_logFilePath.c_str(),
                     info.st_size, errno);
                return;
            }
            ...
            if (fp == nullptr) {
                ALOGE("%s, fopen(a) %s fail, size:%ld, err:%d", __func__, g_logFilePath.c_str(),
                     info.st_size, errno);
                return;
            }
        }
